...
window.onload = function() {
	...
	for (var i = 0; i < monthlySegments; i++) {
		guessSelection.innerHTML = guessSelection.innerHTML + '<div id="' + i.toString() + '" data-date="' + mm.toString() + '-' + yyyy.toString() + '" class="guess-segment ' + (yyyy % 2 == 0 ? "even-year" : "odd-year") + '" style="width: ' + (guessSegmentWidth) + 'px; z-index: ' + (i + 1).toString() + '"><div class="guess-tooltip" style="display: none;"><span class="tooltip-month">' + months[mm] + '</span><br/><span class="tooltip-year">' + yyyy.toString() + '</span></div></div>';
  ...
  for(var i = 0; i < guesses.length; i++) {
    ...
    guess.onclick = function() {
    	...
    	while (j <= correctSegment + 3 && j < monthlySegments) {
    		if (j == guessSegment) {
    			voteTier = 1;
    		}
    	...
    	// tier 2: 9 segments up and down
    	j = (correctSegment >= 9 ? correctSegment - 9 : 0);

    	while (j <= correctSegment + 9 && j < monthlySegments) {
    	...
    	// tier 3: 15 segments up and down
    	j = (correctSegment >= 15 ? correctSegment - 15 : 0);

    	while (j <= correctSegment + 15 && j < monthlySegments) {
    		if (j == guessSegment && voteTier == null) {
    			voteTier = 3;
    		}
    	...
    	// tier 4: 21 segments up and down
    	j = (correctSegment >= 21 ? correctSegment - 21 : 0);

    	while (j <= correctSegment + 21 && j < monthlySegments) {
    		if (j == guessSegment && voteTier == null) {
    			voteTier = 4;
    		}
    	...
    	var flipContainer = document.getElementById("flip-container");
	    	...
	    	shot.setAttribute("src", "");
				...
				document.getElementById("twitter-share-button").setAttribute("data-text", "I scored " + userScore.innerHTML + " on Quizzzle, a Dribbble trivia game.");
